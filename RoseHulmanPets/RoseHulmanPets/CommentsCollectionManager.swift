...
class CommentsCollectionManager{
    
    static let shared = CommentsCollectionManager()
    ...
    func startListening(for documentId: String, changeListener: @escaping (()->Void))->ListenerRegistration{
       ...
       // var query=_collectionRef.order(by: kPhotoBucketLastTouched, descending: true).limit(to: 50)
        ...
        var query=_collectionRef.document(documentId).collection("Comments").order(by: kRosePetsLastTouched, descending: true).limit(to: 50)
        ...
        return query.addSnapshotListener { querySnapshot, error in
            ...
            for document in documents {
                self.latestComments.append(Comments(documentSnapshot: document))
...
//}
    ...
    func add(mq: Posts, caption: String)->String{
    ...
    func editComment(comment:String, commentId:String, photoId:String){
        _collectionRef.document(photoId).collection("Comments").document(commentId).updateData([kComment: comment]) { err in
            if let err = err {
    ...
    func addComment(photoId: String, comment:String, authorName: String, authorPhoto: String){
        var ref: DocumentReference?=nil
        
        ref=_collectionRef.document(photoId).collection("Comments").addDocument(data: ["comment": comment,"authorName": authorName, "lastTouched": Timestamp.init(),"authorPhoto": authorPhoto, kRosePetsAuthorEmail: AuthManager.shared.currentUser?.email]) { err in
            if let err=err{
                print("Error adding document \(err)")
            }else{
                print("document added with id \(ref!.documentID)")
    ...
    func deleteComment(commentId: String, photoId: String){
        _collectionRef.document(photoId).collection("Comments").document(commentId).delete() { err in
            if let err = err {
                print("Error removing document: \(err)")
            } else {
                print("Document successfully removed!")
