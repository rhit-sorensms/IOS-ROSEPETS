...
class MyPostsCollectionManager{
    
    static let shared = MyPostsCollectionManager()
...
//        }
      ...
      var  query=_collectionRef.whereField("authorEmail", isEqualTo: AuthManager.shared.currentUser?.email!)
        query.order(by: kRosePetsLastTouched, descending: true).limit(to: 50)
...
//}
    ...
    func deletePhoto(photoId: String){
        _collectionRef.document(photoId).delete() { err in
            if let err = err {
